<template>
  <div id="blog_author_ing">
        ...
        <div class="author-head-content-wrap">
          ...
          <div class="head-bottom-wrap">
            <div class="head-bottom">
              ...
              <div class="head-bottom-icon">
                <span class="icon iconfont wechat" />
              </div>
              <div class="head-bottom-icon">
                <span class="icon iconfont qq2" />
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="author-body-wrap">
      <div v-for="item in ingObj.cnList" class="author-content-item">
        ...
        <div class="author-content-body-wrap">
          ...
          <div class="board-bottom-wrap">
            <span class="heart-wrap"><span class="icon iconfont heart" /><span>{{ item.digg||"0" }}</span></span>
            <span><span class="icon iconfont Icon-Fixedposition- position" /><span>{{ item.from||"博客园" }}</span></span>
          </div>
        </div>
      </div>
    </div>
    <pagination v-if="ingObj.pageNum" :page-size="ingObj.pageNum" :page-cur="ingObj.current" max-page="10" @clickPage="clickPage" />
...
export default {
  ...
  methods: {
    ...
    askIngList: function(openLoadBar) {
      const self = this
      openLoadBar ? this.BlogHeadBar.emit('barLoadingOpen') : ''
      return new Promise((resolve, reject) => {
        blogApi.loadBlogTalk(this.current).then((data) => {
          self.ingObj = data
          self.ingObj.current = data.pageNum
          openLoadBar ? this.BlogHeadBar.emit('barLoadingClose') : ''
          resolve()
        }).catch(() => {
          resolve()
        })
