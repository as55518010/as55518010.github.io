<template>
  <div id="blog_article_full_screen">
    <transition name="fade">
      <div v-show="isOpenFullScreen" class="blog-article-full-screen-warp">
        <div class="post-body-content-fiex-wrap">
          <div id="full_body_top_target" />
          <div class="post-body-content-fiex-inner-wrap">
            <div class="post-body-title-fullscreen">{{ article.title }}</div>
            <div id="post_body_content_fullscreen" v-highlight v-html="article.body">{{ article.body }}</div>
          </div>
          <div class="post-body-fiex-close">
            <span class="icon iconfont LogonClosed" @click="closeFullScreenEven" />
          </div>
          <div class="post-body-fiex-menu">
            <div id="post_body_fiex_menu_anchor" />
          </div>
          <div id="full_body_bottom_target" />
        </div>
      </div>
    </transition>
  </div>
...
export default {
  ...
  created: function() {
    ...
    this.$baseEventBus.$on('closeFullScreenEven', (dom) => {
      this.isOpenFullScreen = false
    })
  },
  methods: {
    closeFullScreenEven: function() {
      this.BlogHeadBar.emit('closeFullScreenEven', true)
    }
  }
}
</script>
