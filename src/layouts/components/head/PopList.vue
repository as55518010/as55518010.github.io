<template>
  <div id="pop_list">
    <div class="pop-list-wrap small-base-scroll">
      <div v-show="popTitle" class="pop-list-head pop-list-item">
        {{ popTitle }}
      </div>
      <div v-for="(item,id) in popList" class="pop-list-item" @click="clickItem(item,id)">
        <div v-if="flag&&flagIndex==id" class="left-flag" />
        <div class="item-key sing-ellipsis" :class="item.key2?'item-key1':''" v-html="item.key">{{ item.key }}</div>
        <div v-if="item.key2" class="item-key sing-ellipsis item-key2" v-html="item.key2">{{ item.key2 }}</div>
      </div>
    </div>
  </div>
</template>
