    ...
    <div class="music-player-out-wrap">
      <div class="music-player-wrap head-back-color">
        <div class="music-img-wrap" :class="{Rotation:isPlay}">
          <div class="div-img head-music-pic" :style="getPicImgStyle" />
        </div>
        <div class="music-right-wrap">
          <div class="music-ope-wrap">
            <div class="music-title" :style="{width:cssStyle.titleWidth+'px'}" :title="playing.title">
              <div
                class="music-display-info sing-ellipsis"
                :class="{ marquee: isPlay }"
                :style="{width:(cssStyle.titleWidth-5)+'px'}"
              >{{ playing.title }}
              </div>
              <div
                v-show="isPlay"
                class="music-display-info sing-ellipsis"
                :class="{ marqueeT: isPlay }"
                :style="{width:(cssStyle.titleWidth-5)+'px'}"
              >{{ playing.title }}
              </div>
            </div>
            <div class="music-ope-bar">
              <span class="pre" @click="musicPlayPre"><span class="icon iconfont angle-double-left" /></span>
              <span class="play" @click="musicState"><span
                class="icon iconfont "
                :class="{timeout:!isPlay,play1:isPlay}"
              /></span>
              <span class="pos" @click="musicPlayPos"><span class="icon iconfont angle-double-right" /></span>
            </div>
          </div>
          <div class="music-volume-wrap">
            <span class="icon iconfont volume-" />
          </div>
          <div class="music-progress" :style="{width:cssStyle.barWidth+'px'}" />
        </div>
        <div class="music-list-wrap" @click="showPlay=!showPlay">
          <span class="icon iconfont headphones" />
        </div>
      </div>
      <div class="music-player-shade head-back-color" />
...
export default {
  ...
  computed: {
    getPicImgStyle: function() {
      const src = this.playing.pic ? this.playing.pic : BlogContext.musicSignImg
      return {
  ...
  methods: {
    ...
    musicSetInfo: function(item) {
      ...
      const calTitleWidth = blogUtils.getTextWidth(this.playing.name, this.cssStyle.fontSize)
