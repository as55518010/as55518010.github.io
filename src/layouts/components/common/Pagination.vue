  ...
  methods: {
    ...
    computePageList: (pageSize, pageCur, maxPage) => {
      if (pageSize <= maxPage) {
        ...
        /* 前10页*/
        const arr = Array.from(Array(maxPage), (v, k) => k + 1)
        arr.push('...')
        arr.push(pageSize)
        return arr
      } else if ((pageSize - pageCur) <= ((maxPage / 2) + 1)) {
        /* 后10页*/
        const arr = Array.from(Array(maxPage), (v, k) => (pageSize - maxPage) + k + 1)
        arr.unshift('...')
        arr.unshift(1)
        return arr
      } else {
        /* 中间页*/
        const arr = Array.from(Array(maxPage), (v, k) => (pageCur - (maxPage / 2)) + k + 1)
        arr.unshift('...')
        arr.unshift(1)
        arr.push('...')
        arr.push(pageSize)
