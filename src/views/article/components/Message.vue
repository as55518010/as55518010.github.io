        ...
        <div v-for="item in messageList" class="body-item">
          ...
          <div class="message-btn-wrap">
            ...
            <div class="message-btn btn2">
              ...
              />{{ item.burry }}</span>
              <span @click="diggComment(item.commentId)"><span class="icon iconfont zan" />{{ item.digg }}</span>
...
export default {
  ...
  created: function() {
    this.initMyCommentList(this.articleId, -1)
    this.$baseEventBus.$on('commentCommitEvent', () => {
      this.initMyCommentList(this.articleId, -1)
    })
    this.$baseEventBus.$on('commentUpdateEvent', () => {
      this.initMyCommentList(this.articleId, -1)
    })
  },
  methods: {
    ...
    initMyCommentList: function(articleId, pageNum) {
      ...
      blogApi.loadMyCommentList(articleId, pageNum).then((data) => {
        ...
        this.BlogHeadBar.emit('barLoadingClose')
      })
    }
  }
}
