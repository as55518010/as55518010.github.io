  ...
  <div id="left">
    <el-card class="box-card">
      ...
      <div v-for="(item) in List" :key="item.id" class="content">
        <div class="img">
          <img :src="item.img" alt="图片" @click="detailPage(item.article_id)">
        </div>
        <div class="text">
          ...
          <div class="text-bottom">
            <div class="creater">
              ...
              <span class="name"><i class="el-icon-user" />{{ item.name }}</span>
              <span class="time"><i class="el-icon-time" />{{ item.create_time | formateDate }}</span>
              <!-- <span @click="changeLike(item.like_Star, item.id, index)"><i class="iconfont icon-dianzan"></i>{{item.like_Star}}</span> -->
              <span><i class="iconfont icon-pinglun" />{{ item.replyCount }} 条评论</span>
              <span><i class="iconfont icon-zhiboguankanshu" />{{ item.visited }}</span>
            </div>
            <div class="tag">
              <el-tag><i class="iconfont icon-biaoqian" style="font-size:.8rem;margin-right:.5rem;" />{{ item.label }}</el-tag>
            </div>
          </div>
        </div>
      </div>
