...
const blogApi = {
  ...
  loadCommitterFaceUrl: (committerUrl, committerName) => {
    const pidSplTmp = committerUrl.split('/')
    const pid = pidSplTmp[pidSplTmp.length - 1].replace('.html', '')
  ...
  loadMyCommentList: (articleId, page) => {
    return new Promise((resolve, reject) => {
      const mySize = BlogContext.myCommentSize
      const blogSize = BlogContext.blogCommentSize
      blogApi.loadCommentCount(articleId).then((r1) => {
        const maxPage = Math.ceil(r1 / mySize)
        page = (page == -1 ? maxPage : page)
        const myScale = blogSize / mySize
        const blogPage = Math.ceil(page / myScale)
        const blogSideLeft = (parseInt((page - 1) % myScale) * 10)
